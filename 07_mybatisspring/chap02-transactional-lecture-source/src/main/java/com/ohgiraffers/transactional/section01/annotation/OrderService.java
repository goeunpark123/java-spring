...
public class OrderService {
    ...
    /*설명. OrderDTO에 담겨있을 내용:
    *  Service 계층부터 개발할 때는 사용자가 입력한 값들이 어떻게 DTO 또는 Map으로 묶여서
    *  Service로 넘어올지 충분히 고민한 후 매개변수를 작성하고 개발
    *  현재의 경우 사용자가 고른 메뉴들 각각의 코드와 고른 메뉴 개수, 그리고 서버의 현재 시간이 담긴 채로 넘어왔다는 생각을 가지고 개발*/

    /*설명. 서비스 메소드 내부에서 모든 DML 관련 작업에 예외 없이 잘 동작하면 commit, 예외가 발생하면 rollback이 적용*/
...
//        );
        ...
        /*설명 5. tbl_order_menu에도 주문한 메뉴 개수만큼 주문한 메뉴 추가*/
        int orderMenuSize = oMenus.size();
        for (int i = 0; i < orderMenuSize; i++) {
            OrderMenu orderMenu = oMenus.get(i);
            orderMenu.setOrderCode(order.getOrderCode());           // entity에 setter 필요에 따라서 추가

...
public class LoggingAspect {
    ...
    /*설명.
    *  타켓 클래스의 메소드에서 어드비아시를 적용할 수 있는 지점들을 조인 포인트(joinpoint)라고 한다.
    *  포인트컷(pointcut)은 여러 조인 포인트들에 어드바이스를 적용할 곳을 지정한 것이다.
    *  해당 조인포인트에서 어드바이스가 동작한다.*/

    /*설명.
    *  <포인트컷 표현식> execution([수식어] 리턴 타입 [클래스 이름]. 이름(파라미터))
    ...
    *  => com.ohgiraffers.section01.advice 패키지 및 하위 패키지에 속해있고 (클래스 상관 X) 이름이 stu로 시작하는 파라미터가 0개 이상인
    *     모든 메소드이자 접근 제어자와 반환형은 상관 없음
    * */
    ...
    /*설명 1. Before Adivice*/
    /*설명.
    *  매개변수로 쓰인 JoinPoint: 포인트 컷으로 패치된 조인 포인트*/
...
//        System.out.println("Before Advice 동작");
        ...
        if(joinPoint.getArgs().length > 0) {        // 매개변수가 하나 이상
...
//        System.out.println("After Advice 동작");
    ...
    /*설명. returning에 쓰인 이름이 (반환값으로 넘어오는) 매개변수명과 일치해야 한다.(result)*/
    ...
    /*설명 5. Around Advice*/
    /*설명. 앞서 살펴본 어드바이스 모두 Around 어드바이스로 조립 가능
    *  원본 조인 포인트의 실행 시점, 실행 여부, 계속 실행할지 여부까지 제어
    *  AroundAdvice의 조인 포인트 매개변수는 Proceeding.JoinPoint로 고정
    *  JoinPoint의 하위 인터페이스로 원본 조인 포인트의 진행 시점 제어 가능
    *  조인 포인트를 진행하는 호출을 잊는 경우가 자주 발생하기 때문에 주의
    *  최소한의 요건을 충족하면서도 가장 기능이 약한 어드바이스 사용이 바람직 (자원 효율 문제)*/
    @Around("logPointcut()")
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable{
        ...
        /*설명. 실행된 타켓 메소드 반환(다른 어드바이스가 다시 실행할 수 있도록 반환)*/
        return result;
    }
    // joinpoint 중에 하나인 pointcut
}

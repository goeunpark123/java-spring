...
public class ProblemOfUsingDirectSQLTests {
    ...
    /*설명. JDBC API 이용해 직접 SQL를 다룰 때 발생할 수 있는 문제점
    *  1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)
    *  2. SQL에 의존하여 개발
    *  3. 패러다임 불일치(상속, 연관 관계, 객체 그래프 탐색, 방향성)
    *  4. 동일성 보장 문제*/

    /*설명 1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)*/
    @DisplayName("직접 SQL 작성하여 메뉴 조회 시 발생하는 문제 확인")
    @Test
    ...
    @Test
    void testDirect() throws SQLException {
        // given
        Menu menu = new Menu();
   ...
   /*설명 2. SQL에 의존하여 개발*/
    ...
    /*설명 3. 패러다임 불일치(상속, 연관 관계, 객체 그래프 탐색, 방향성)*/
    /*설명 3-1. 상속 문제
    *  객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브타입 엔티티(서브타입을 별도의 클래스로 나뉘었을 때)
    *  슈퍼타입의 모든 속성을 서브타입이 공유하지 못하여 물리적으로 다른 테이블로 분리된 형태
    *  (설계에 따라서는 하나의 테이블로 속성이 추가되기도 한다.)
    *  하지만 객체 지향의 상속은 슈퍼 타입의 속성을 공유해서 사용하므로 여기에서 패러다임의 불일치 발생*/

    /*설명 3-2. 연관관계 문제, 그래프 탐색 문제
    *  객체지향에서 말하는 가지고 있는(ASSOCIATION 연관 관계 혹은 COLLECTION 연관 관계) 경우 데이터베이스 저장 구조와 다른 형태 */

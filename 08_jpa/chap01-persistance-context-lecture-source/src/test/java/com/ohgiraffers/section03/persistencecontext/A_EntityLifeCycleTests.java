...
public class A_EntityLifeCycleTests {
    ...
    /*필기.
    *  영속성 컨텍스트는 엔티티 매니저가 엔티티 객체를 저장하는 공간으로 앤티티 객체를 보관하고 관리
    *  엔티티 매니저가 생성될 때 하나의 영속성 컨텍스트가 만들어진다.
    * */

    /*필기.
    *  엔티티의 생명 주기: 비영속, 영속, 준영속, 삭제*/
    @Test
...
//        entityManager.clear();
    ...
    public void 병합_merge_수정_테스트() {
        Menu menuToDetatch = entityManager.find(Menu.class, 2);
        entityManager.detach(menuToDetatch);

        menuToDetatch.setMenuName("수박죽");
        Menu refoundMenu = entityManager.find(Menu.class, 2);       // refoundMenu에는 기존의 메뉴 이름 존재

        System.out.println(menuToDetatch.hashCode());
        System.out.println(refoundMenu.hashCode());

        entityManager.merge(menuToDetatch);

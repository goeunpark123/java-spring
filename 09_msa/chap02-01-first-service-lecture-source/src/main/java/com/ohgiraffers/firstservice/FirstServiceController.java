...
public class FirstServiceController {
    ...
    public String healthCheck() {
        ...
        /*설명. first-service 애플리케이션 여러 개 실행 시(스케일 아웃) 실행되는 애플리케이션 확인(포트 번호로)*/
        /*설명. Gateway의 lb(로드 밸런싱)을 통해 RoundRobin 방식으로 스위칭됨을 확인*/
        log.info("요청 경로 포트1: {}", env.getProperty("local.server.port"));
//        log.info("요청 경로 포트2: {}", env.getProperty("server.port"));
        return "check";
    }

    /*설명. Gateway를 거치고 넘어오는 요청 시 RequestHeader에 값 추가(Gateway의 필터에 의해)*/
    @GetMapping("message")
